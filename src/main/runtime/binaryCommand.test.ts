...
describe('binaryCommand', () => {
  it('runs Windows command shims through ComSpec without enabling a shell in spawn', () => {
    expect(binaryCommand('C:\\Program Files\\Bingo\\bingo.cmd', ['--json-events', '--probe'], 'win32', 'C:\\Windows\\System32\\cmd.exe')).toEqual({
      command: 'C:\\Windows\\System32\\cmd.exe',
      args: ['/d', '/s', '/c', '""C:\\Program Files\\Bingo\\bingo.cmd" --json-events --probe"'],
      windowsVerbatimArguments: true
    })
  })

  it('runs JavaScript entry points with the current Node executable', () => {
    expect(binaryCommand('/opt/bingo.mjs', ['--json-events'], 'linux')).toEqual({ command: process.execPath, args: ['/opt/bingo.mjs', '--json-events'] })
  })
