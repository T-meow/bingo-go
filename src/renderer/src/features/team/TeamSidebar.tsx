...
export function TeamSidebar({ snapshot, selection, onSelect }: {
  ...
  if (!snapshot) return <div className="sidebar-content"><header className="sidebar-title"><div><span>协作</span><strong>Team</strong></div></header><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="等待 Team 数据" /></div>
  ...
  return <div className="sidebar-content team-sidebar">
    ...
    <section className="team-nav-section"><h2>频道</h2>{channelItems.length
      ? <Conversations rootClassName="team-conversations" activeKey={activeKey} items={channelItems} onActiveChange={select} />
      : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚未启动频道" />}</section>
    <section className="team-nav-section"><h2>成员</h2>{memberItems.length
      ? <Conversations rootClassName="team-conversations" activeKey={activeKey} items={memberItems} onActiveChange={select} />
      : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="没有成员" />}</section>
