...
vi.mock('electron', () => ({
  contextBridge: { exposeInMainWorld: (_name: string, api: BingoGuiApi) => { electron.api = api } },
  ipcRenderer: {
...
describe('preload notification bridge', () => {
  ...
  it('validates notification preferences before invoking main', async () => {
    ...
    await electron.api!.saveNotificationPreferences(input)

    expect(electron.invoke).toHaveBeenCalledWith(IPC.notificationPreferencesSave, input)
    expect(() => electron.api!.saveNotificationPreferences({ ...input, values: { ...input.values, enabled: 'yes' } } as never)).toThrow()
  })

  it('forwards only validated activation events and unsubscribes exactly', () => {
    const listener = vi.fn()
    const unsubscribe = electron.api!.onNotificationActivated(listener)
  ...
  it('validates team-task commands before forwarding them to main', async () => {
    ...
    await electron.api!.createTeamTask(input)

    expect(electron.invoke).toHaveBeenCalledWith(IPC.teamTaskCreate, { ...input, contextMessageSeqs: [], additionalConstraints: [] })
    expect(() => electron.api!.getTeamTask({ connectionId, taskId: 'task-1', limit: 201 })).toThrow()
  })
  ...
  it('validates clipboard writes and session forks before invoking main', async () => {
    ...
    await electron.api!.writeClipboardText({ text: '可见文字' })
    await electron.api!.forkSession(input)
    ...
    expect(() => electron.api!.forkSession({ ...input, sourceRevision: undefined })).toThrow()
    expect(() => electron.api!.forkSession({ sourceSessionId: 'source-session', reason: 'recover-interrupted', sourceTurnId: input.sourceTurnId })).toThrow()
  })

  it('exposes only validated game-package commands and filters events', async () => {
    const revision = 'f'.repeat(64)
    electron.invoke.mockResolvedValue({ ok: true })
    await electron.api!.installGamePack({ token: '123e4567-e89b-42d3-a456-426614174000', baseRevision: revision })
    await electron.api!.setGamePackEnabled({ id: 'com.example.game', enabled: false, baseRevision: revision })
    await electron.api!.launchGamePack({ id: 'com.example.game' })
    ...
    expect(() => electron.api!.launchGamePack({ id: '../unsafe' })).toThrow()

    const listener = vi.fn()
    const unsubscribe = electron.api!.onGamePackEvent(listener)
