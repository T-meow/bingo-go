...
const CELL = 40
const PALETTES = [
  ['#E9F2FF', '#2463A7'], ['#FDEDEE', '#B73D4C'], ['#EBF7EF', '#287A4B'],
  ['#FFF3DE', '#A55B16'], ['#F1ECFA', '#7252A3'], ['#E8F6F7', '#22747B'],
  ['#F7EDEF', '#A34669'], ['#EEF0F4', '#52617A'], ['#F2F6E8', '#637D28'],
  ['#FFF0E8', '#B45129'], ['#E9F3F1', '#356E64'], ['#F4EDF7', '#80508D']
]

const outputDirectories = process.argv.slice(2).map((path) => resolve(path))
if (outputDirectories.length === 0) outputDirectories.push(resolve('src/renderer/public/avatars'))
...
for (let index = 0; index < PALETTES.length; index += 1) {
  ...
  await Promise.all(outputDirectories.map((directory) => writeFile(resolve(directory, `${id}.png`), data)))
}
...
function renderIdenticon(id, [background, foreground]) {
  ...
  for (let row = 0; row < GRID; row += 1) {
    const half = []
    for (let column = 0; column < 3; column += 1) half.push(seed[row * 3 + column] < 146)
    cells.push([half[0], half[1], half[2], half[1], half[0]])
  }
  const active = cells.flat().filter(Boolean).length
  if (active < 8) cells[2] = [true, true, true, true, true]
  const color = hex(foreground)
  for (let row = 0; row < GRID; row += 1) {
    for (let column = 0; column < GRID; column += 1) {
      if (cells[row][column]) rect(pixels, PADDING + column * CELL, PADDING + row * CELL, CELL, CELL, color)
    }
  }
  return png(pixels)
}
...
function rect(pixels, x, y, width, height, color) {
  for (let row = y; row < y + height; row += 1) {
    for (let column = x; column < x + width; column += 1) pixels.set(color, (row * SIZE + column) * 4)
  }
}
